...
const Dashboard = () => {
  ...
  const fetchPortfolio = useCallback(async () => {
    try {
      ...
      const response = await axios.get(
        `https://kripto-bag.onrender.com/api/portfolio/${userId}`
      );
          ...
          response.data.map(async (coin) => {
            try {
              const priceResponse = await axios.get(
                `https://kripto-bag.onrender.com/api/coin-price/${coin.id}`
              );
              return {
                ...coin,
  ...
  const fetchCoinsData = useCallback(async () => {
    try {
      ...
      const popularResponse = await axios.get(
        "https://kripto-bag.onrender.com/api/popular-cryptocurrencies"
      );
      ...
      if (Array.isArray(popularResponse.data)) {
        ...
        const allResponse = await axios.get(
          "https://kripto-bag.onrender.com/api/cryptocurrencies"
        );
  ...
  const getCurrentPrice = async (coinId) => {
    try {
      ...
      const response = await axios.get(
        `https://kripto-bag.onrender.com/api/coin-price/${coinId}`
      );
  ...
  const removeCoinFromPortfolio = async (coinId) => {
    try {
      ...
      await axios.delete(
        `https://kripto-bag.onrender.com/api/portfolio/${userId}/${coinId}`
      );
  ...
  const fetchCoinDetails = async (coinId) => {
    try {
      ...
      const [detailsResponse, chartResponse] = await Promise.all([
        axios.get(`https://kripto-bag.onrender.com/api/coin/${coinId}`),
        axios.get(
          `https://kripto-bag.onrender.com/api/coin/${coinId}/market_chart?days=1`
        ),
      ]);
