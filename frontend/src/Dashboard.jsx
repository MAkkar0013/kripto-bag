...
const Dashboard = () => {
  ...
  const fetchPortfolio = useCallback(async () => {
    try {
      ...
      const coins = response.data.slice(0, 10); // Ограничиваем 10 монетами

      const portfolioWithPrices = await Promise.all(
        coins.map(async (coin) => {
          try {
            const priceResponse = await axios.get(`/api/coin-price/${coin.id}`);
            return {
              ...coin,
  ...
  const fetchCoinsData = useCallback(async () => {
    try {
      ...
      const popularResponse = await axios.get("/api/popular-cryptocurrencies");
      ...
      if (Array.isArray(popularResponse.data)) {
        ...
        // Затем все монеты
        const allResponse = await axios.get("/api/cryptocurrencies");
  ...
  const getCurrentPrice = async (coinId) => {
    try {
      ...
      const response = await axios.get(`/api/coin-price/${coinId}`);
  ...
  const addCoinToPortfolio = async () => {
    ...
    try {
      ...
      await axios.post(`/api/portfolio/${userId}`, {
  ...
  const removeCoinFromPortfolio = async (coinId) => {
    try {
      ...
      await axios.delete(`/api/portfolio/${userId}/${coinId}`);
  ...
  const fetchCoinDetails = async (coinId) => {
    try {
      ...
      const [detailsResponse, chartResponse] = await Promise.all([
        axios.get(`/api/coin/${coinId}`),
        axios.get(`/api/coin/${coinId}/market_chart?days=1`),
      ]);
